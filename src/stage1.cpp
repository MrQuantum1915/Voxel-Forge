...
// code implementation taken from openMVG/src/software/SfM/main_SfMInit_ImageListing.cpp
// repo
...
namespace OpenMVG_Wrappers
{
    ...
    bool RunImageListing(
        ...
        bool b_Group_camera_model,
        bool b_Use_pose_prior,
        const std::string &sPriorWeights,
        int i_GPS_XYZ_method)
    {

        auto LOG = [&](const std::string &msg)
        {
            OPENMVG_LOG_INFO << msg; // Console logging
            if (logCallback)
                logCallback(msg); // Qt UI logging
        };
        ...
        {
            LOG_ERROR("The input directory doesn't exist");
            return false;
        }
        ...
        {
            LOG_ERROR("Invalid K matrix input");
            return false;
        }

        if (sKmatrix.size() > 0 && focal_pixels != -1.0)
        {
            LOG_ERROR("Cannot combine -f and -k options");
            return false;
        }
        ...
        if (!sSensorDb.empty())
        {
            if (!parseDatabase(sSensorDb, vec_database))
            {
                LOG_ERROR("Invalid input database: " + sSensorDb + ", please specify a valid file.");
                return false;
            }
        }
        ...
        {
            LOG_WARNING("Warning & Error messages:\n" + error_report_stream.str());
        }
        ...
        LOG("SfMInit_ImageListing report:\n"
            "listed #File(s): " + std::to_string(vec_image.size()) + "\n"
            "usable #File(s) listed in sfm_data: " + std::to_string(sfm_data.GetViews().size()) + "\n"
            "usable #Intrinsic(s) listed in sfm_data: " + std::to_string(sfm_data.GetIntrinsics().size()));
        return true;
    }
}